...
if len(sys.argv) != 8:
    print('Arguments: nem_features_path, S, P, K, Dataset_name [vgg_faces|ilsvrc|google_landmarks|places365], first_b, last_b')
    sys.exit(-1)
#Parameters###############################
batch_size = 256
...
for b in range(first_batch, last_batch + 1):
    ...
    #train
    reduced_train_images_paths_file = os.path.join(images_list_files_path,dataset+'/S~'+S+'/unbalanced/train/K~'+K+'/'+str(b+1)+'_old')
    #Val
