...
nb_proto = int(cp['nb_proto'])  # Number of groups
...
print('normalization_dataset_name = ' + str(normalization_dataset_name))
output_file.write('normalization_dataset_name = ' + str(normalization_dataset_name) + '\n')
...
print('nb_proto = ' + str(nb_proto))
output_file.write('nb_proto = ' + str(nb_proto) + '\n')
...
for itera in range(nb_groups):
    ...
    if itera == 0:
        ...
        for i in range(itera * nb_cl): # pour toutes les classes anterieures
            ...
            tmp_var = files_protoset[i] #recuperer la listes des exemplaires de cette classe
            files_from_cl += tmp_var[0:min(len(tmp_var), nb_protos_cl)] #ajouter les exemplaires (paths) disponibles a la liste courante de train
    ...
    if itera == 0:
        # No distillation
        variables_graph, variables_graph2, scores, scores_stored = utils_icarl.prepare_networks(images_mean, device,
                                                                                                image_batch, nb_cl,
                                                                                                nb_groups)
    ...
    with tf.Session(config=config) as sess:
        ...
        for iter_dico in range(nb_cl):
            ...
            # print('class '+str(iter_dico)+' : '+str(len(files_iter)))
            if len(files_iter) <= nb_protos_cl:
                print('Not enough exemplars for class '+str(iter_dico))
                for v in range(len(files_iter)):
                    files_protoset[itera * nb_cl + iter_dico].append(files_iter[v])
            else:
