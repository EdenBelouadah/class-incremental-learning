...
old_train_images_list_dir = cp['old_train_images_list_dir']
...
with warnings.catch_warnings(record=True) as warn_list:
    ...
    for b in range (first_batch_number, last_batch_number+1):
        ...
        for i in range(len(full_file_paths)):
            if full_file_paths[i] in features_dict: #voir s'il y a une image repetee deux fois dans le fichier d'entree
    ...
    #########################################################
        ...
        for data_type in data_types:
            ...
            if data_type == 'old':
                images_list = os.path.join(old_train_images_list_dir,  str(b)+'_'+data_type)
            else:
            ...
            full_features = None  # np.empty([512, ])
            full_scores = None  # np.empty([512, ])
            full_file_paths = None  # np.empty([1, ])
