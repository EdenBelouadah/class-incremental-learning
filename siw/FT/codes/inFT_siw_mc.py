...
for batch_number in range(2, S + 1):
    ...
    for i in range(np_model_weights.shape[0]):
        # np_model_weights[i] -= np.mean(np_model_weights[i])
        # np_model_weights[i] /= np.std(np_model_weights[i])
...
###############################################################################################
    ...
    #Accuracy
    print('[batch {}] Before Calibration | test : acc@5 = {:.1f}%'.format(batch_number,  top5.avg))
    print('[batch {}] After Calibration  | test : acc@5 = {:.1f}%'.format(batch_number,  top5_rectified.avg))
    # print('***********************************************************************')
