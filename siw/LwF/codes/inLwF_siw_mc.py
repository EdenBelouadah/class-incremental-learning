...
batch_train_mean_score = {k : 1 for k in range(S + 1)}
batch_train_counter =    {k : 1 for k in range(S + 1)}
...
for batch_number in range(1, S + 1):
    ...
    train_images_paths_file = os.path.join(scores_path,dataset + '/S~' + str(S) + '/K~' + memory_size + '/train/batch' + str(batch_number) + '/paths_features.lst')
    train_images_features_file = os.path.join(scores_path,dataset + '/S~' + str(S) + '/K~' + memory_size + '/train/batch' + str(batch_number) + '/features.raw')
    ...
    assert (len(train_images_paths_file) == len(train_images_features_file))
    assert(len(val_images_paths_file) == len(val_images_features_file) )


    ######################## compute mean train score:
    for (train_path_line, train_feat_line) in zip(train_images_paths_file, train_images_features_file):
        ...
        train_np_feat = np.array(train_feat_line, dtype=np.float).reshape(1, -1)
        train_np_scores = train_np_feat.dot(np_model_weights) + np_model_bias
        if train_image_class >= (batch_number - 1) * P and train_image_class < batch_number * P:
            batch_train_counter[batch_number] += 1
            batch_train_mean_score[batch_number] += np.max(train_np_scores)
    ...
    for i in range(rectified_np_model_weights.shape[1]):
        mu = np.mean(rectified_np_model_weights[:, i])
        std = np.std(rectified_np_model_weights[:, i])
        rectified_np_model_weights[:, i] -= mu
        rectified_np_model_weights[:, i] /= std
    ...
    for (val_path_line, val_feat_line) in zip(val_images_paths_file, val_images_features_file):
        ...
        for o in range((batch_number - 1) * P): #for all past classes
            rectified_val_np_scores[0][o] *=  batch_train_mean_score[batch_number] / batch_train_mean_score[int(o / P) + 1]
